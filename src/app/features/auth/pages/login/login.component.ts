...
export class LoginComponent {
  ...
  recordarme = signal(true); // checkbox "Recordarme" -> localStorage vs sessionStorage
  ...
  abrirRecuperacion(): void {
    ...
    const correoLogin = this.loginForm.get('email')?.value || '';
  ...
  toggleRecordarme() {
    this.recordarme.update((val) => !val);
  }
  ...
  onSubmit() {
    ...
    setTimeout(() => {
      ...
      this.authService.login(loginRequest).subscribe({
        next: (response) => {
          // Redirige según rol, o a returnUrl si venía de un guard
  ...
  enviarRecuperacion(): void {
    ...
    // Validar vacío
    if (!correo) {
      this.errorRecuperacion.set('Ingrese su correo electrónico.');
      return;
    }
    ...
    if (!formatoCorreo.test(correo)) {
      this.errorRecuperacion.set('Ingrese un correo electrónico válido.');
      return;
    }
    ...
    this.authService.forgot(request).subscribe({
      next: (response) => {
        ...
        this.mensajeRecuperacion.set(response.mensaje);

        console.log('Recuperación enviada:', response);
      },
      ...
      error: (error) => {
        ...
        if (error.status === 404) {
          this.errorRecuperacion.set('No existe un usuario registrado con ese correo.');
        } else {
          this.errorRecuperacion.set('No se pudo enviar el correo. Intente nuevamente.');
        }
      },
    });
  }
